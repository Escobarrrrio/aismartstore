...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <article data-testid="product-card" data-product-id={product.id} data-product-category={product.category ?? ''} className="@container group relative flex flex-col overflow-hidden rounded-2xl bg-card border border-border/60 hover:border-border transition-all duration-300 hover:shadow-[0_20px_50px_-20px_hsl(var(--foreground)/0.18)] hover:-translate-y-0.5">
      {/* Image */}
      <Link
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        {/* Price + CTA */}
        <div className="flex flex-col @[220px]:flex-row @[220px]:items-end @[220px]:justify-between gap-3 pt-3 border-t border-border/60">
          ...
          <button
            ...
            className={`h-10 w-full @[220px]:w-auto rounded-full text-sm font-semibold flex items-center justify-center gap-1.5 px-4 transition-all flex-shrink-0 ${
              addedToCart
