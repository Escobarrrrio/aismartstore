-- ===========================================================================
-- "AI Smart Store" header text in the AI Pulse email -- pearl white instead
-- of plain white, permanently
-- ===========================================================================
--
-- Requested after comparing the two template variants that went out before
-- the ambiguous-overload fix (20260813000000): the surviving daily template
-- (this function) is the one preferred. The only change asked for on it is
-- the wordmark colour in the gradient header -- currently flat #fff, wanted
-- as a softer "pearl white" so it sits better against the cyan/purple/
-- magenta gradient behind it. Everything else in the function is unchanged
-- from the version that survived 20260813000000's overload cleanup.
-- ===========================================================================

CREATE OR REPLACE FUNCTION public.build_ai_pulse_digest(
  p_stories int DEFAULT 5,
  p_min_stories int DEFAULT 3,
  p_min_score numeric DEFAULT 55,
  p_max_per_source int DEFAULT 2
)
RETURNS uuid
LANGUAGE plpgsql
      ...
      || '<div style="background:linear-gradient(135deg,#06b6d4 0%,#7c3aed 50%,#d946ef 100%);padding:22px 28px">'
        || '<table role="presentation" cellpadding="0" cellspacing="0"><tr>'
          ...
          || '<td style="vertical-align:middle">'
            -- Pearl white (#f7f5f2), not flat #fff -- reads softer against the
            -- cyan/purple/magenta gradient behind it. Intentionally kept apart
            -- from the country-pill's own #fff (that badge stays flat white).
            || '<div style="font-size:17px;font-weight:800;color:#f7f5f2;letter-spacing:-.01em">AI Smart Store</div>'
          || '</td>'
        || '</tr></table>'
      || '</div>'
